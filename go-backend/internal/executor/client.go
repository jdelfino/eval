...
// TestRequest is an alias for the shared test request type.
type TestRequest = executorapi.TestRequest

// TestResponse is an alias for the shared test response type.
type TestResponse = executorapi.TestResponse

// TestResult is an alias for the shared test result type.
type TestResult = executorapi.TestResult

// TestSummary is an alias for the shared test summary type.
type TestSummary = executorapi.TestSummary

// IOTestDef is an alias for the shared I/O test definition type.
type IOTestDef = executorapi.IOTestDef

// RunTests sends code + I/O test definitions to the executor service and returns structured results.
func (c *Client) RunTests(ctx context.Context, req TestRequest) (*TestResponse, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, fmt.Errorf("executor: marshal test request: %w", err)
	}

	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+"/test", bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("executor: create test request: %w", err)
	}
	httpReq.Header.Set("Content-Type", "application/json")
	if reqID := chimiddleware.GetReqID(ctx); reqID != "" {
		httpReq.Header.Set("X-Request-ID", reqID)
	...
	httpResp, err := c.httpClient.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("executor: send test request: %w", err)
	}
	defer func() { _ = httpResp.Body.Close() }()
	...
	respBody, err := io.ReadAll(io.LimitReader(httpResp.Body, 5<<20))
	if err != nil {
		return nil, fmt.Errorf("executor: read test response: %w", err)
	}
	...
	if err := json.Unmarshal(respBody, &resp); err != nil {
		...
		return nil, fmt.Errorf("executor: decode test response: %w (body: %s)", err, snippet)
	}
