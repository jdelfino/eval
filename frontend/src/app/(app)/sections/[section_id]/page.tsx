...
export default function SectionDetailPage() {
  ...
  const handleProblemClick = async (problemId: string) => {
    try {
      const work = await getOrCreateStudentWork(section_id, problemId);
  ...
  const handleActiveSessionJoin = async () => {
    ...
    try {
      const work = await getOrCreateStudentWork(section_id, session.problem.id);
      router.push(`/student?work_id=${work.id}`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Published Problems */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Published Problems</h2>
