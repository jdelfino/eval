...
export default function UserList({ users, currentUserId, onDelete, showActions = false }: UserListProps) {
  ...
  if (users.length === 0) {
    return (
      <div className="py-8 text-center text-[var(--fg-muted)] bg-[var(--bg-sunken)] rounded">
        No users found
      </div>
    );
  }
  ...
  return (
    <Table className="bg-[var(--bg-raised)] shadow-sm">
      <Table.Header>
        <Table.Row>
      ...
      <Table.Body>
        {users.map((user) => (
          <Table.Row key={user.id}>
            ...
            <Table.Cell className="text-sm text-[var(--fg-muted)]">
              {user.email}
            </Table.Cell>
            <Table.Cell>
