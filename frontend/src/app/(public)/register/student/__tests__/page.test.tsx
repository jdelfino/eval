...
describe('StudentRegistrationPage', () => {
  ...
  describe('Auth State Change Listener', () => {
    it('auto-proceeds with registration when Firebase auth state changes to signed-in during code-valid state', async () => {
      // Mock onAuthStateChanged to synchronously call the callback with a Firebase user
      // This simulates signInWithEmailAndPassword being called in the browser (E2E scenario)
      const mockFirebaseUser = { uid: 'firebase-uid', email: 'test@example.com', getIdToken: jest.fn() };
      ...
      mockCurrentUser = null;
      mockGetStudentRegistrationInfo.mockResolvedValue({
        section: { id: 'sec-2', name: 'Auth Change Section' },
        class: { id: 'cls-2', name: 'Auth Change Class' },
      });
      mockRegisterStudent.mockResolvedValue({ id: 'user-2', role: 'student' });
      ...
      // Wait for the section preview to appear (code-valid state)
      await waitFor(() => {
        expect(screen.getByText('Auth Change Class')).toBeInTheDocument();
      });

      // The onAuthStateChanged callback fires with a user, which should trigger registration
      await waitFor(() => {
        expect(mockRegisterStudent).toHaveBeenCalledWith('ABC-123');
      });

      await waitFor(() => {
        expect(mockPush).toHaveBeenCalledWith('/sections/sec-2');
      });
    });

    it('does not double-register if auth state change fires multiple times', async () => {
      ...
      mockCurrentUser = null;
      mockGetStudentRegistrationInfo.mockResolvedValue({
        section: { id: 'sec-3', name: 'Double Call Section' },
        class: { id: 'cls-3', name: 'Double Call Class' },
      });
      mockRegisterStudent.mockResolvedValue({ id: 'user-3', role: 'student' });
      ...
      await waitFor(() => {
        expect(screen.getByText('Double Call Class')).toBeInTheDocument();
        expect(capturedCallback).not.toBeNull();
      });

      const mockUser = { uid: 'uid-3', email: 'test3@example.com' };

      // Simulate auth state change firing multiple times (e.g. token refresh)
