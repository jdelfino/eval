...
describe('useApiDebugger', () => {
  ...
  describe('requestTrace', () => {
    it('calls traceCode with code and sets trace', async () => {
      ...
      await act(async () => {
        await result.current.requestTrace('print("hello")', 'python', { kind: 'io' as const, name: 'default', input: '', match_type: 'exact', order: 0 });
      });

      expect(mockTraceCode).toHaveBeenCalledWith('print("hello")', 'python', { kind: 'io' as const, name: 'default', input: '', match_type: 'exact', order: 0 });
    ...
    it('sets isLoading during request', async () => {
      ...
      act(() => {
        promise = result.current.requestTrace('code', 'python', { kind: 'io' as const, name: 'default', input: '', match_type: 'exact', order: 0 });
      });
    ...
    it('handles error from traceCode', async () => {
      ...
      await act(async () => {
        await result.current.requestTrace('code', 'python', { kind: 'io' as const, name: 'default', input: '', match_type: 'exact', order: 0 });
      });
    ...
    it('passes IOTestCase to traceCode', async () => {
      ...
      const testCase = { kind: 'io' as const, name: 'default', input: 'hello', match_type: 'exact' as const, order: 0, random_seed: 42, attached_files: [{ name: 'f.txt', content: 'data' }] };
      await act(async () => {
    ...
    it('propagates trace.error from response', async () => {
      ...
      await act(async () => {
        await result.current.requestTrace('bad code', 'python', { kind: 'io' as const, name: 'default', input: '', match_type: 'exact', order: 0 });
      });
  ...
  describe('step navigation', () => {
    async function setupWithTrace() {
      ...
      await act(async () => {
        await hook.result.current.requestTrace('code', 'python', { kind: 'io' as const, name: 'default', input: '', match_type: 'exact', order: 0 });
      });
      return hook;
    }
