...
# the IAP SSH tunnel (or any reachable HTTP endpoint).
#
# Usage:
#   ./scripts/executor-smoke-test.sh [BASE_URL]
#
# Environment:
#   BASE_URL  (default: http://localhost:8080)  — base URL of the proxy/executor
#
...
if [[ "$health_code" != "200" ]]; then
  ...
  echo "  Is the IAP tunnel active and the executor running?"
  exit 1
fi
