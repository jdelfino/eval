...
assert_contains \
  "defaults BASE_URL to http://localhost:8080" \
  "localhost:8080"
...
# ────────────────────────────────────────────────────────────────────────────
# Behavior: missing BASE_URL still uses default (http://localhost:8080)
...
# Run the script without BASE_URL — expect it to try localhost:8080
default_url_output=$(
  env -i \
...
if echo "$default_url_output" | grep -qE "localhost:8080|localhost"; then
  echo "PASS: script uses localhost:8080 as default BASE_URL"
  PASS=$((PASS + 1))
else
  echo "FAIL: script did not use localhost:8080 as default BASE_URL"
